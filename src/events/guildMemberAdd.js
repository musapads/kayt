...
member.guild.channels.cache.get(config.channels.welcomechannel).send({ embeds: [new MessageEmbed()
 .setDescription(`<a:leise_tada:997202772841730048> **Bir üye Aramıza Katıldı.!**
        
 ${member} **Kimleri görüyorum!** 
  
 <a:gul2:994204659726159902> **Seninle beraber ${member.guild.memberCount} kişiyiz!**
 
 <:123:997465835297853540> **Kayıt olmak için <@&${config.registration.staff}> rolündeki yetkilileri etiketlimeyi unutma.**
  
  **Hesap Oluşturma Tarihi**
  \`${kuruluş}\`
  
  **Bu hesap (${zaman})**
  <a:tikPayitaht:997505875432456282> \`Güvenilir!\`
 `)], content: `<@&${config.registration.staff}> ${member}`});
    } else {
        member.setNickname(config.registration.susoeciosnickname);
        member.roles.add(config.registration.suspecios);
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send({ embeds: [new MessageEmbed()
.setDescription(`${member}, kullanıcısı sunucuya katıldı hesabı **${zaman2}** önce açıldığı için şüpheli.`)], content: `<@&${config.registration.staff}> ${member}`})
    }
} 
